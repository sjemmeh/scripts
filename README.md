...
| [`manage.sh`](Server%20Related/Deployment/manage.sh) | Interactive RHEL Podman web-app manager — deploys/restores customers and updates customer Docker images in bulk. |
...
`manage.sh` stores app registrations in `/root/.config/webvm/ports` as:

```text
name port customer|standalone
```

Bulk Docker image updates only run for `customer` entries. Existing two-column registry rows are treated as `customer` for backward compatibility.
